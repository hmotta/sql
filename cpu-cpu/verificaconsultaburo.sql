...
begin
	select coalesce(max(cb.consultaid),0),max(fecha) into r.consultaid,r.ultimafecha from consultaburo cb,respuestaburo rb where paterno=ppaterno and materno=pmaterno and primer_nombre=pprimer_nombre and segundo_nombre=psegundo_nombre and fecha_nacimiento=pfecha_nacimiento and substr(rfc,1,10)=substr(prfc,1,10) and cb.consultaid=rb.consultaid;
