CREATE or replace FUNCTION spssaldoadeudolinea(integer) RETURNS numeric
    AS $_$
declare
...
begin
	...
	select sum(mp.debe),sum(mp.haber) into fcargos,fabonos from movipolizas mp,prestamos p,cat_cuentas_tipoprestamo ct  where mp.prestamoid=p.prestamoid and (ct.cat_cuentasid = p.cat_cuentasid) and p.prestamoid=lprestamoid and (mp.cuentaid = ct.cta_cap_vig);
