...
-- Validar apertura de caja
    ...
    select serie_user,cuentacaja
      into fserie_user,fcuentacaja
      from parametros
...
----
   ...
   update autorizabonificacion set movicajaid=imovicajaid,aplicado=1 where autorizacionid=iautorizacionid;
	...
	-- >> Validar Folios de P3 hmota-2012/04/10
	if ptipomovimientoid='P3' then 
	   select sum(mp.debe)-sum(mp.haber) into fmontomovp3 from movicaja mc, movipolizas mp, tipomovimiento tm where  mp.movipolizaid=mc.movipolizaid and mc.tipomovimientoid='P3' and tm.tipomovimientoid=mc.tipomovimientoid and tm.aplicasaldo='S';
	   
	   update foliops set vigente='N' where socioid=psocioid and tipomovimientoid='P3';
	   
	   if fmontomovp3>0 then
			select ejercicio,periodo into iejercicio,iperiodo from poliza where polizaid=ppolizaid;
			...
			insert into foliops(socioid,movicajaid,tipomovimientoid,ejercicio,periodo,folioini,foliofin,vigente) values(psocioid,imovicajaid,'P3',iejercicio,iperiodo,ifolioini,ifoliofin,'S');
	   end if;
