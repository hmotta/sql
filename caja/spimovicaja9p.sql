...
CREATE FUNCTION spimovicaja(integer, character, integer, integer, character, integer, integer, character, integer) RETURNS integer
    AS $_$
declare
...
--     raise exception 'El socio debe pasar primeramente a información a realizar su BAJA Antes de realizar el retiro de su PARTE SOCIAL';
--   end if;
   ...
   if saplicasaldo='S' and
      fretiro>fsaldo and fretiro>0 and 
   ...
   --if ptipomovimientoid='AS' and fsaldo+fdeposito > 1000 then
   --   raise exception 'El socio no pude depositar más de $1000 en ASPIRANTE';
   --end if;

   --if ptipomovimientoid='PA' and fsaldo+fdeposito <> 1000 then
   --   raise exception 'El socio solo puede depositar $1000 en Parte social';
   --end if;

   -- Validar la promocion
   if not validapromocion(psocioid,ptipomovimientoid,fretiro) then
     raise exception 'El socio no pude realizar retiro por que esta en una promoción';
   end if;
   ...
   insert into movicaja(socioid,tipomovimientoid,polizaid,referenciacaja,seriecaja,movipolizaid,prestamoid,estatusmovicaja,inversionid,saldoactual,efectivo,fechahora)
   values(psocioid,ptipomovimientoid,ppolizaid,preferenciacaja,pseriecaja,pmovipolizaid,pprestamoid,pestatusmovicaja,pinversionid,fsaldo,pefectivo,current_timestamp);

------------------------------------------

-- Impuesto Deposito en efectivo

   if saplicasaldo='S' and ptipomovimientoid<>'IN' and ptipomovimientoid<>'RM' and ptipomovimientoid<>'RE' and pefectivo=1 then
     ...
     if fsumaefectivo > fideexento then
      ...
      insert into movicaja(socioid,tipomovimientoid,polizaid,referenciacaja,seriecaja,movipolizaid,prestamoid,estatusmovicaja,inversionid,saldoactual,efectivo,fechahora)
      values(psocioid,ptipomovimientoid,ppolizaid1,preferenciacaja,pseriecaja,pmovipolizaid1,NULL,pestatusmovicaja,NULL,0,0,current_timestamp);
