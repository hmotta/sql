CREATE OR REPLACE FUNCTION cortecajatransfer(character, date, integer) RETURNS SETOF rcortecaja
    AS $_$
declare
 pserie alias for $1;
 pfecha alias for $2;
...
for l in
 ...
 loop 
   ...
   select coalesce(sum(coalesce(haber,0)),0) into fnormal
     from movipolizas
    where polizaid = l.polizaid and
          cuentaid = l.cuentaintnormal;

   select coalesce(sum(coalesce(haber,0)),0) into fmoratorio
     from movipolizas
    where polizaid = l.polizaid and
          cuentaid = l.cuentaintmora;

   select coalesce(sum(coalesce(haber,0)),0) into fiva
     from movipolizas
    where polizaid = l.polizaid and
          cuentaid = l.cuentaiva;

   select coalesce(sum(coalesce(haber,0)),0) into fcobranza
     from movipolizas
    where polizaid = l.polizaid and
...
ALTER FUNCTION public.cortecajatransfer(character, date, integer) OWNER TO sistema;
