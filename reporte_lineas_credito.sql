...
begin
	...
	for r in
      select 
		...
		(select max(po.fechapoliza) from polizas po,movipolizas mp,prestamos p1,cat_cuentas_tipoprestamo ct  where po.polizaid=mp.polizaid and mp.prestamoid=p1.prestamoid and (ct.tipoprestamoid = p1.tipoprestamoid and ct.clavefinalidad = p1.clavefinalidad and ct.renovado = p1.renovado) and mp.haber>0 and p1.prestamoid=p.prestamoid and (mp.cuentaid = ct.cuentaactivo) and po.fechapoliza<=pfecha),
