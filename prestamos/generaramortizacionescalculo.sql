...
begin
  ...
  update prestamos set calculonormalid=pcalculoid where prestamoid=pprestamoid;
  ...
  loop
      for r in
        select * from
           sptablaamorcalculo(x.montoprestamo,x.fecha_1er_pago,x.tipoprestamoid,x.numero_de_amor,
                       x.dias_de_cobro,x.meses_de_cobro,x.dia_mes_cobro,x.fecha_otorga,x.tasanormal,pcalculoid)
      loop
