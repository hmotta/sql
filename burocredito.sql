...
begin
  ...
  for r in
       ...
       --4 paterno	
       (case when su.paterno = null or su.paterno = ' ' or su.paterno = '' or su.paterno = 'X' then 'NO PROPORCIONADO' else sburo(su.paterno) end),
       --5 materno
       (case when su.materno = null or su.materno = ' ' or su.materno = '' or su.materno = 'X' then 'NO PROPORCIONADO' else sburo(su.materno) end),
       --6 adicional
       '',
       --7 primer_nombre
       sburo(su.nombre),
       ...
       '',
       --16 calle_numero
       sburo(sburo(d.calle)||' '||sburo(d.numero_ext)),
       --17 direccion_complemento
       '',
       --18 colonia_o_pob
       (case when character_length(sburo(col.nombrecolonia))>40 then '' else sburo(col.nombrecolonia) end),
       --19 municipio
       sburo(c.nombreciudadmex),--rtrim(d.comunidad),
       --20 ciudad
       ...
       (case when pr.saldoprestamo=0 or (pr.prestamoid=9952 and p.saldoprestamo=0 and s.socioid=3415) then to_char(pr.ultimoabono,'DDMMYYYY') else '' end),
       --48 fecha_reporte
       to_char(pfecha,'DDMMYYYY'),
       --49 credito_maximo
       trunc(p.montoprestamo),
       --50 saldo_actual El saldo m?todos los intereses.
       ...
       (case when pr.diascapital>=15 and (trunc(importevencidoamort)>0) then pr.noamorvencidas else 0 end),
       --54 forma_pago_mop
	   (case when pr.diascapital>=15 then(case when (trunc(pr.importevencidoamort))=0 then '01' else (case when pr.diascapital >=1 and pr.diascapital <=29 then '02' else (case when pr.diascapital >29 and pr.diascapital <=59 then '03' else (case when pr.diascapital >59 and pr.diascapital <=89 then '04' else (case when pr.diascapital >89 and pr.diascapital <=119 then '05' else  (case when pr.diascapital >119 and pr.diascapital <=149 then '06' else (case when pr.diascapital >149 and pr.diascapital <=360 then '07' else (case when pr.diascapital >360 and pr.diascapital <=389 then '96' else '96' end) end) end)end) end) end) end) end) else '01' end),
       ...
       where pr.fechacierre = pfecha and  s.tiposocioid = '02' and so.personajuridicaid = 0 and 
			 --p.prestamoid not in (6895,8327,3399,3398,3356,3357,3358,3359,3355,2705,2703) and
             p.prestamoid = pr.prestamoid and 
			 p.tipoprestamoid <> 'CAS' and
			 substring(trim(clavesocioint),13,1)<>'C' and
  ...
  loop
	...
	--Verificar Creditos castigados
	select saldoprestamo into saldocastigado from prestamos where referenciaprestamo = substr(r.cuenta,4,6)||'-CAS-'; 
	if found then
		r.forma_pago_actual:='96';
		r.observacion:='UP';
		r.saldo_vencido:=trunc(saldocastigado);
		r.saldo_actual:=trunc(saldocastigado);
	end if;
	
	select prestamoid into pprestamoid from prestamos where referenciaprestamo=substr(r.cuenta,4,6)||'-S' and saldoprestamo=0;
	--Poner la clave de reestructuras en automatico
	if found then
		r.observacion:='RV';
	end if; 
	...
	for l in
		select 
			(case when su.paterno = null or su.paterno = ' ' or su.paterno = '' or su.paterno = 'X' then 'NO PROPORCIONADO' else sburo(su.paterno) end) as paterno,
			(case when su.materno = null or su.materno = ' ' or su.materno = '' or su.materno = 'X' then 'NO PROPORCIONADO' else sburo(su.materno) end) as materno,
			sburo(su.nombre) as nombre,
			trim(to_char(su.fecha_nacimiento,'DDMMYYYY')) as fechanacimiento,
			substring(trim(su.rfc),1,10)  as rfc,
			sburo(sburo(d.calle)||' '||sburo(d.numero_ext))  as direccion,
			(case when character_length(sburo(col.nombrecolonia))>40 then '' else sburo(col.nombrecolonia) end) as colonia,
			sburo(c.nombreciudadmex) as poblacion,
			trim(e.abre_estado) as estado,
			lpad(trim(to_char(col.cp,'99999')),5,'0') as codigop
		...
		where su.sujetoid=a.sujetoid and 
			...
			a.prestamoid = (select prestamoid from prestamos where referenciaprestamo=substr(r.cuenta,4,6)||'-')
	  loop
		--delete from carteraclaveburo where prestamoid=l.prestamoid;
		--insert into carteraclaveburo (prestamoid,clave) values (l.prestamoid,'RV');
		r.paterno:=l.paterno;
		...
		IF pposicion > 3 THEN
			...
			r.segundo :=substring(pnombre,pposicion+1,character_length(pnombre));
		END IF;
		r.fecha_nac:=l.fechanacimiento;
		r.rfc:=l.rfc;
		r.calle_numero:=l.direccion;
		r.colonia:=l.colonia;
		r.municipio:=l.poblacion;
		r.estado:=l.estado;
		r.cp:=l.codigop;
		r.edo_civil:='';
		r.sexo:='';
