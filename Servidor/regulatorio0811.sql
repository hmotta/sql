...
begin
  ...
  if pmiles = 0 then 
     select 0,round(sum(intdevacumulado)),round(sum(intdevacumulado+intdevmensual)),round(sum(intdevmensual)),0 into a1,b1,c1,d1,e1  from captaciontotal  where fechadegeneracion=pfechaf and tipomovimientoid in ('AO','AF');

     select round(sum(deposito)),round(sum(intdevacumulado)),round(sum(intdevacumulado+intdevmensual)),round(sum(intdevmensual)),0 into a2,b2,c2,d2,e2  from captaciontotal  where fechadegeneracion=pfechaf and tipomovimientoid in ('AM','AI');

     select round(sum(deposito)),round(sum(intdevacumulado)),round(sum(deposito+intdevacumulado)),round(sum(intdevmensual)),0 into a3,b3,c3,d3,e3  from captaciontotal  where fechadegeneracion=pfechaf and tipomovimientoid in ('IN');

  else
  
     select 0,round(sum(intdevacumulado)),round(sum(intdevacumulado+intdevmensual)),round(sum(intdevmensual)),0 into a1,b1,c1,d1,e1  from captaciontotal  where fechadegeneracion=pfechaf and tipomovimientoid in ('AF','AM','AI');
     select round(sum(deposito)),round(sum(intdevacumulado)),round(sum(intdevacumulado+intdevmensual)),round(sum(intdevmensual)),0 into a2,b2,c2,d2,e2  from captaciontotal  where fechadegeneracion=pfechaf and tipomovimientoid in ('AM','AI');
     select round(sum(deposito)),round(sum(intdevacumulado)),round(sum(deposito+intdevacumulado)),round(sum(intdevmensual)),0 into a3,b3,c3,d3,e3  from captaciontotal  where fechadegeneracion=pfechaf and tipomovimientoid in ('IN');
