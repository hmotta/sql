...
trazonesreporte:='';
 ...
 -- Procesando Operacion
  ...
  --MONTO                   numeric, 
  (case when ptiporegistro in (2,3) then 0 else (select coalesce(round(sum(valor),2),0) from sabana where referenciacaja =preferenciacaja and seriecaja=pseriecaja and entradasalida=0) end) as monto,   --14
  --MONEDA                  char(3), 
  'MNX', --15
