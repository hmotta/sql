	...
	if ptipomovimientoid='P3' then 
	   ...
	   if fmontomovp3>0 then
		...
		if ptipomovimientoid='IN' then
			...
			select tipoinversionid,inversionanteriorid into stipoinversionid,pinversionanteriorid from inversion where inversionid=pinversionid;
			if stipoinversionid in ('PSO','PSV') THEN
				--if pinversionanteriorid=NULL then --Es una inversion nueva
