...
begin
    ...
    for r in
		...
		(case when d.teldomicilio= ' ' then '000000000000000' else d.teldomicilio  end) as  teldomicilio,
		(select (case when celular= ' ' then '000000000000000' else celular  end) as  celular from extensionsujeto  where sujetoid=su.sujetoid)
	from 	socio s,
		sujeto  su,
		solicitudingreso si,
		domicilio d,
