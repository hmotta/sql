...
-- Encabezado de la poliza
  ...
  select p.tipoprestamoid,t.cuentaactivo,t.cuentaactivovencida,t.cuentaintdevnocobres,
       ...
       sum((case when p.fechacierre= pfechaactual
...
group by p.tipoprestamoid,t.cuentaactivo,t.cuentaactivovencida,t.cuentaintdevnocobres,
       ...
       t.cuentaintnormal,t.cuentaintmora,t.cuentaintnormalresvencida,
       t.cuentaintnormalvencida,t.cuentaintmoravencida,t.cuentaintnormalresvigente,
       t.cuentariesgocred,t.cuentariesgocredres--,p.factoraplicado 
  loop

    for r1 in
  select t.tipoprestamoid,
       sum((case when p.fechacierre=pfechaanterior
...
-- Encabezado de la poliza
  ...
  --Cierre Actual
  for r in
	  select p.tipoprestamoid,t.moravigentebalance,t.moravencidobalance,t.moravigenteresultado,
       t.moravencidoresultado,t.moractaordendeudora,t.moractaordenacredora,
       sum((case when p.diasvencidos <= p.diastraspasoavencida
                 then p.interesdevmormenor else 0 end)) as MoratorioVigente,
       sum((case when p.diasvencidos > p.diastraspasoavencida
                 then p.interesdevmormenor else 0 end)) as MoratorioVencido,
       sum((case when p.diasvencidos > p.diastraspasoavencida
                 then p.interesdevmormayor else 0 end)) as MoratorioCtasOrden       
	   --sum((case when (p.reservacalculada+p.reservaidnc+p.reservagarantia)*p.factoraplicado > 0
                 --then (p.reservacalculada+p.reservaidnc+p.reservagarantia)*p.factoraplicado else 0 end)) as reservacalculada   
  from precorte p, tipoprestamo t
  where p.fechacierre = pfechaactual and p.tipoprestamoid <> 'CAS' and t.tipoprestamoid = p.tipoprestamoid 
  group by p.tipoprestamoid,t.moravigentebalance,t.moravencidobalance,t.moravigenteresultado,
       t.moravencidoresultado,t.moractaordendeudora,t.moractaordenacredora--,p.factoraplicado 
  loop
	  --Cierre Anterior
	  for r1 in
		  select t.tipoprestamoid,
		   sum((case when p.fechacierre=pfechaanterior and p.diasvencidos <= p.diastraspasoavencida
					 then p.interesdevmormenor else 0 end)) as MoratorioVigenteAnt,
		   sum((case when p.fechacierre=pfechaanterior and p.diasvencidos > p.diastraspasoavencida
					 then p.interesdevmormenor else 0 end)) as MoratorioVencidoAnt,
		   sum((case when p.fechacierre=pfechaanterior and p.diasvencidos > p.diastraspasoavencida
					 then p.interesdevmormayor else 0 end)) as MoratorioCtasOrdenAnt       
		   --sum((case when (p.reservacalculada+p.reservaidnc+p.reservagarantia)*p.factoraplicado > 0
					 --then (p.reservacalculada+p.reservaidnc+p.reservagarantia)*p.factoraplicado else 0 end)) as reservacalculada   
	  from tipoprestamo t left join precorte p on t.tipoprestamoid=p.tipoprestamoid
