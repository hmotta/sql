CREATE OR REPLACE FUNCTION carteracrediticiaxsuc(integer, integer, character) RETURNS SETOF tcarteracrediticia
    AS $_$
declare
...
for f in
 select * from sucursales where vigente='S' and basededatos = pbase
 loop
  ...
  dblink2:='set search_path to public,'||f.esquema||';
     select s.clavesocioint,pr.referenciaprestamo,pr.tipoprestamoid,p.ejercicio,p.periodo,p.fechacierre,
