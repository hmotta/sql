...
CREATE OR REPLACE FUNCTION carteracrediticiasioef(integer, integer) RETURNS SETOF tcarteracrediticia
   AS $_$
declare
...
begin
	    for r in
            ...
            interesdevmormenor,interesdevmormayor,
                        ...
                        (case when p.diasvencidos > p.diastraspasoavencida and p.tipoprestamoid in ('R1','R2','R3') then 'Renovado' end) end) end) end) end) end) as estacion,(case when p.dias_de_cobro > 0 then p.dias_de_cobro else p.meses_de_cobro*30 end),' ',
...
(select  trim(ffinalidad(fl.finalidad,fl.subfinalidad1,fl.subfinalidad2)) AS desfinalidad   from finalidad fl,solicitudprestamo sp  where fl.solicitudprestamoid=sp.solicitudprestamoid and pr.solicitudprestamoid=sp.solicitudprestamoid) ,
