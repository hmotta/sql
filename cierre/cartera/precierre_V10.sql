...
select p.prestamoid,
       ...
       ( case when pr.revolvente=0 then fechaultimapagada(p.prestamoid,pfechacorte) else NULL end) as fechaultamorpagada,
	   
       p.tipoprestamoid,p.montoprestamo,tp.clavefinalidad,p.tasanormal,p.tasa_moratoria,
	   MAX(case when (m.cuentaid=tp.cuentaactivo or m.cuentaid=tp.cuentaactivoren) and po.fechapoliza<dcorte then po.fechapoliza else p.fecha_otorga end) AS ultimoabono,
...
--<<--
	...
	for r in 
		select prestamoid from prestamos where socioid in (select socioid from precorte p inner join prestamos pr on (p.prestamoid=pr.prestamoid) where p.tipo_cartera_est='tipo 2' and fechacierre=pfechacorte)
	loop
