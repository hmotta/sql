...
begin
	...
	if ncapitalapagar>0 then --creditos que ya debieron o deben pagar capital 
		...
		if ncapitaltablaamort=ncapitalpolizas then
			select coalesce(pfecha-min(fechadepago),0) into ndias from amortizaciones where abonopagado<>importeamortizacion and prestamoid=pprestamoid and fechadepago<=pfecha; --se toman los dias de la tabla de amort
			if ndias<0 then
				return 0;
			end if;
		else
