...
begin
  ...
  precorte (prestamoid,ejercicio,periodo,fechacierre,saldoprestamo,
            ...
            ultimoabonointeres,interesdevmormenor,interesdevmormayor,dias_de_cobro,meses_de_cobro,frecuencia,finalidaddefault,fecha_otorga,estatusvive,depositogarantia,diasvencidos)
...
group by p.prestamoid, p.tipoprestamoid,p.montoprestamo,
         ...
         p.fecha_1er_pago,tp.diastraspasoavencida,p.fecha_vencimiento,p.dias_de_cobro,p.meses_de_cobro,tp.clavefinalidad,p.fecha_otorga,p.monto_garantia,p.claveestadocredito,p.numero_de_amor
   order by prestamoid;
   ...
   loop
     ...
     if r.saldoprestamo>0 then
		   if pfechacorte-r.ultimoabonointeres>0 then
				 ...
				 --
				finteresdevengadomenor := interesdevengado(r.prestamoid,pfechacorte,r.ultimoabonointeres,r.saldoprestamo,r.diastraspasoavencida,'S');
				...
				fdevengadomayor := interesdevengado(r.prestamoid,pfechacorte,r.ultimoabonointeres,r.saldoprestamo,r.diastraspasoavencida,'N');
			...
			-- Asignar a los dias vencido lo que sea mayor interes o capital 
