...
-- Actualizar interes pagado
  ...
  select sum(case when m.cuentaid=ct.cta_cap_vig then m.haber else 0 end) as interes into finteres from movicaja mc, movipolizas m, prestamos pr,  cat_cuentas_tipoprestamo ct where mc.prestamoid=pprestamoid and mc.tipomovimientoid='00' and pr.prestamoid = mc.prestamoid and (ct.cat_cuentasid = pr.cat_cuentasid) and m.polizaid = mc.polizaid;
