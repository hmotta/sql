CREATE OR REPLACE FUNCTION carteracrediticiasioefc(integer, integer) RETURNS SETOF tcarteracrediticia1
    AS $_$
declare
...
for f in
 ...
 loop
  ...
  raise notice 'Conectando sucursal % % ',f.basededatos,f.esquema;
        ...
        dblink2:='set search_path to public,'||f.esquema||'; select * from carteracrediticiasioef('||pejercicio||','||pperiodo||');';
