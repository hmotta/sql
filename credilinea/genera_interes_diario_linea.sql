CREATE or replace FUNCTION genera_interes_diario_linea(integer,date) RETURNS integer
    AS $_$
declare
...
begin
	...
	if dfecha_inicial is null then
		...
		--raise notice 'Tomando fecha de ultimo corte pagado...';
		...
		--if dfecha_inicial is null then --No hay cortes pagados entonces se toma entonces se toma la fecha de la primera disposicion que tuvo y apartir de ahi se empieza a calcular el interes
			...
			raise notice 'Tomando fecha de primera disp...';
		--end if;
	...
	for i in 1..ndias_interes loop
		...
		insert into credito_linea_interes_devengado (lineaid,fecha,saldo,interes_diario) values (pprestamoid,dfecha,xsaldo,xinteres_diario);
	end loop;
	return 1;
end
$_$
