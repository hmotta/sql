		...
		if ftotalhaberes < fcapital then
			...
			if ftotalhaberes - fcapital - finteres > 0 then --Logra cubrir todo el capital + el interes ord. y parte de moratorio
				...
				select * into pmovipolizaid4
		  			from spimovipoliza(ppolizaid,scuentaintmora,' ','A',0,fmoraaplicado,' ',' ','CASTIGO PRESTAMO');
				select * into pmovipolizaid5
					  from spimovipoliza(ppolizaid,scuentaiva,' ','A',0,fivamoraaplicado,' ',' ','CASTIGO PRESTAMO');
			end if;
...
-- ********************* Encabezado de la poliza 2 ***************************** --
	  ...
	  select  p.montoprestamo-sum(m.haber) into fcapitalapĺicado  from prestamos p, tipoprestamo tp, movicaja mc, movipolizas m where p.prestamoid = pprestamoid and tp.tipoprestamoid = p.tipoprestamoid and mc.prestamoid = p.prestamoid and m.polizaid = mc.polizaid and m.cuentaid = tp.cuentaactivo group by p.saldoprestamo,p.montoprestamo;

  --Verificar Operacion (saldo del nuevo prestamo calculado)
  pcapitalpagado:= pmontoprestamo - fcapitalapĺicado;
  raise notice 'pcapitalpagado  % =  pmontoprestamo % - fcapitalapĺicado %',pcapitalpagado,pmontoprestamo,fcapitalapĺicado;
  select * 
    into ppolizaid
    from spipolizasfecha(preferencia,pserie_user,'A',pnumero_poliza,cast(date_part('year',pfecha) as int),cast(date_part('month',pfecha) as int),' ',pfecha,'D',' ',' ','CASTIGO DE PRESTAMO:'||sreferenciaprestamo,pfecha);
...
-- >>>  Movimientos de la poliza 2 <<<<--  
	...
	if fcapitalapĺicado > 0 then
		select *
	       into pmovipolizaid
	       from spimovipoliza(ppolizaid,scuentariesgocred,' ','C',fcapitalapĺicado,0,' ',' ','CASTIGO PRESTAMO');
	   
		select *
	       into pmovipolizaid2
	       from spimovipoliza(ppolizaid,scuentaactivo,' ','A',0,fcapitalapĺicado,' ',' ','CASTIGO PRESTAMO');
	end if;
