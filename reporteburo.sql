...
begin
  ...
  for r in
	   ...
	   (select max(p1.fechapoliza) from polizas p1 natural join movipolizas mp1 inner join prestamos pr1 on (mp1.prestamoid=pr1.prestamoid) inner join cat_cuentas_tipoprestamo ct on (ct.cat_cuentasid = pr1.cat_cuentasid) where mp1.prestamoid=p.prestamoid and (mp1.cuentaid = ct.cuentaactivo) and p1.fechapoliza<=pfecha and mp1.debe>0),
  ...
  loop
	...
	if r.fecha_otorga >= '2011-05-25' then
		if r.revolvente=1 then
			...
			select fechadepago into r.fecha_primer_incum from amortizaciones where dias_mora_capital>0 and prestamoid=r.prestamoid and fechadepago<=pfecha order by fechadepago limit 1;
