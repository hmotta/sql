...
begin
	...
	select sum(mp.debe),sum(mp.haber) into fcargos,fabonos from polizas po,movipolizas mp,prestamos p,cat_cuentas_tipoprestamo ct  where po.polizaid=mp.polizaid and mp.prestamoid=p.prestamoid and (ct.cat_cuentasid = p.cat_cuentasid) and p.prestamoid=lprestamoid and (mp.cuentaid = ct.cta_cap_vig) and po.fechapoliza<=pfecha;
