...
begin
	    for r in
            ...
            interesdevmormenor,interesdevmormayor,
						...
						(case when (select count(*) from precorte p1 where p1.renovado=1 and p1.diasmoraorigen>0 and socioid=p.socioid and p1.fechacierre=p.fechacierre)>=1 then 'Renovado 2' else
                        ...
                        (case when p.diasvencidos > p.diastraspasoavencida and p.tipoprestamoid in ('R1','R2','R3') then 'Renovado' end) end) end) end) end) end) end) end) end) as estacion,
						(case when p.dias_de_cobro > 0 then p.dias_de_cobro else p.meses_de_cobro*30 end),
						' ',
...
(select  fondeadora   from carteradisposicion cd, disposicion d  where cd.disposicionid=d.disposicionid and cd.prestamoid=pr.prestamoid ) as fondeadora,
	...
	(select  * from spsgarantiaprestamo(pr.prestamoid)) as tipogar
      from precorte p,prestamos pr,socio s,tipoprestamo t, sujeto su, solicitudingreso si, domicilio d,
           ciudadesmex c, finalidades f, cat_cuentas_tipoprestamo ct
     where p.ejercicio=pejercicio and p.periodo=pperiodo and pr.tipoprestamoid <> 'CAS' and  pr.prestamoid=p.prestamoid and 
           ...
           c.ciudadmexid = d.ciudadmexid and f.clavefinalidad=p.clavefinalidad  and 
