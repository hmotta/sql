...
begin
  ...
  x1:=trunc(((capitalneto/requerimientototal)*100),4);
  x2:=trunc((( capitalneto/(b3-b4))*100),4);
  x3:=(( capitalneto/(requerimientoa+b3-b4))*100);
...
--
  ...
  r.t1 := (round(c2)+round(c9)+round(b8))*0.08;
  r.t2 := NULL;
  r.t3 := NULL;
  r.t4 := 1;
