...
-- Validar los creditos reestructurados 
   ...
   for r in
     select p.precorteid,p.saldoprestamo,p.fechaultamorpagada,p.prestamoid,p.tipoprestamoid,
            p.ultimoabono,p.diastraspasoavencida,p.frecuencia,p.pagosostenido
   ...
   loop
     ...
     --else
		...
		if r.pagosostenido<3 then
			...
			idiasvencidos:=idiasvencidos+(select diasmoraorigen from prestamos where prestamoid=r.prestamoid);
		end if;
