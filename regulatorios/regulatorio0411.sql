...
CREATE FUNCTION cartera411c(integer,integer) RETURNS SETOF tcartera411
    AS $_$
declare
...
for f in
 ...
 loop
        ...
        dblink2:='set search_path to public,'||f.esquema||';select * from cartera411('||pejercicio||','||pperiodo||');';
		  for r in
		    select * from
...
select pagosvencidos,finalidaddefault,round(sum(saldoprestamo)) as tsaldo1,round(sum(interesdevengadomenoravencido+interesdevmormenor)) as tsaldo2,round(sum(saldoprestamo)+sum(interesdevengadomenoravencido+interesdevmormenor)) as tsaldo3, round(sum(pagointeresenperiodo)) as tsaldo4 from precorte where ejercicio=pejercicio and periodo=pperiodo and tipoprestamoid<>'CAS' group by pagosvencidos,finalidaddefault order by pagosvencidos,finalidaddefault 
loop
