--Modificada en Agosto 2018 por ctas bloqueadas
--Bit
...
--   raise exception '�Tu primer movimiento del d�a debe una dotaci�n';
--   end if;
------< Termino de validar primer movimiento RE

---->> Inicio a validar que no hagan remesa sino han entregado todos sus cr�ditos.*/
...
-----<< Termino de validar que no hagan remesa sino han entregado todos sus cr�ditos.*/
...
RAISE NOTICE 'VALIDARE GARANTIAAAAAAA';
	...
	if (ngarantiaactual-fretiro)<ngarantiarequerida then 
	 --  raise exception 'El monto en garantía no puede ser menor a: %',round(ngarantiarequerida,2);"COMENTE PARA PODER REINVERTIR"
	end if;
    end if;
		...
		if ptipomovimientoid='IN' then
			...
			if stipoinversionid in ('PSO','PSV') THEN
				...
				--Retiro
					...
					update foliops set inversionid=pinversionid where socioid=psocioid and tipomovimientoid=stipoinversionid and inversionid=pinversionanteriorid;
				end if;
			end if;
		end if;
	end if;
...
-- linea Original if ptipomovimientoid in ((select tipomovimientoid from tipomovimiento where tipomovimientoid<>'CI' and tipomovimientoid<>'IP' and ptipomovimientoid<>'AH' and  tipomovimientoid<>'RE' and aplicasaldo='S') union (select (case when exists (select socioid from datosfiscales where socioid =psocioid) then '**' else '00' end) )) and  pefectivo=1 then
   if ptipomovimientoid in ((select tipomovimientoid from tipomovimiento where tipomovimientoid<>'CI' and ptipomovimientoid<>'AH' and  tipomovimientoid<>'RE' and aplicasaldo='S') union (select (case when exists (select socioid from datosfiscales where socioid =psocioid) then '**' else '00' end) )) and  pefectivo=0 then
     ...
     if fsumaefectivo > fideexento then
      ...
      values(psocioid,'ID',ppolizaid1,preferenciacaja,pseriecaja,pmovipolizaid1,NULL,pestatusmovicaja,NULL,0,0);
      ...
      insert into movicaja(socioid,tipomovimientoid,polizaid,referenciacaja,seriecaja,movipolizaid,prestamoid,estatusmovicaja,inversionid,saldoactual,efectivo)
      values(psocioid,ptipomovimientoret,ppolizaid1,preferenciacaja,pseriecaja,pmovipolizaid1,NULL,pestatusmovicaja,NULL,0,0);*/
