...
CREATE TYPE tcaptacioncomite AS (
	...
	-- Columnas de control
	intdevacumulado_periodo_anterior numeric,
	intdevacumulado_periodo_actual numeric,
	intdevmensual_ahorros numeric,
	intdevmensual_inversiones numeric,
	intpagadoreal numeric
);
...
declare
  ...
  l tcaptacioncomite%rowtype;
  psucid char(4);
  fecha_perido_anterior date;

begin
fecha_perido_anterior:=pfechacierre-cast(date_part('day',pfechacierre) as int);
raise notice ' Fecha Inicial: % ',fecha_perido_anterior;
...
-- Columnas de control
	...
	(case when tipomovimientoid in ('IN') then intdevacumulado else 0 end),
	--Dev Mensual Ahorros
	(case when tipomovimientoid not in ('IN') then intdevmensual else 0 end),
	--Dev Mensual Inversiones
	(case when tipomovimientoid in ('IN') then intdevmensual else 0 end),
	0
	from captaciontotal 
...
--Periodo Anterior 
	--Para sacar el interes acumulado mensual anterior y el interes realmente pagado
for l in
select
--01-ClaveSocio
	Rtrim(clavesocioint),
...
-- Columnas de control
	...
	0,
	0,
	(SELECT coalesce(sum(debe),0)  FROM polizas p, movipolizas m WHERE p.fechapoliza between fecha_perido_anterior+1 AND pfechacierre AND m.polizaid = p.polizaid AND m.cuentaid in ('5102020101','5102020108') and inversionid = captaciontotal.inversionid)
	from captaciontotal 
	where fechadegeneracion=fecha_perido_anterior 
	and sucursal=psucid and tipomovimientoid not in ('IP','ID')
	order by clavesocioint, tipomovimientoid
...
--
-- Name: spscaptacioncomiteautorizacion(date); Type: FUNCTION; Schema: public; Owner: sistema
--
...
ALTER FUNCTION public.spscaptacioncomitec(date) OWNER TO sistema;

--
-- Name: spscaptacionpatmir(date, date); Type: FUNCTION; Schema: public; Owner: sistema
--
