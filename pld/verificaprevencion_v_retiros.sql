...
declare
	...
	stipomovanterior char(2);
	stipomovposterior char(2);
	...
	tdatosretrelevante text;
	tdatosrelevantecred text;
	...
	begin
		...
		tdatospreocupante:='';
		filtro := 'NORMAL';
		raise notice 'SerieCaja=%',pseriecaja;
	   raise notice 'ReferenciaCaja=%',preferenciacaja;
		...
		if stipomovimientoid not in ('RE') then
			...
			select coalesce(sum(valor),0) into pretiro from sabana where referenciacaja =preferenciacaja and seriecaja=pseriecaja and entradasalida=1;
			select tipomovimientoid into stipomovanterior from movicaja where referenciacaja=(preferenciacaja-1) and seriecaja=pseriecaja;
			if stipomovanterior<>'CH' then
			...
			select tipomovimientoid into stipomovposterior from movicaja where referenciacaja=(preferenciacaja+1) and seriecaja=pseriecaja;
			if stipomovanterior<>'CH' then
			...
			if pdeposito > pmontorelevante and pdeposito<>nretiroinversion then 
				filtro := 'RELEVANTE';
				if stipomovimientoid='00' then
					tdatosrelevantecred:=' MOV PAGO CRED '||stipomovimientoid||' MONTO '||round(pdeposito,0);
				else
			...
			--Tomar en cuenta los retiros
			if pretiro > pmontorelevante  then 
				filtro := 'RETIRO RELEVANTE';
				tdatosretrelevante:=' RET MOV '||stipomovimientoid||' MONTO '||round(pdeposito,0);
				--tdatosrelevante:=' MOV '||stipomovimientoid||' MONTO '||round(pdeposito,0);
				--select * into igenerado from spioperacionlavado(4,0,imovicajaid,0,0,tdatosrelevante);
			end if;
			...
			if trim(coalesce(tdatosretrelevante,''))<>'' then    
				select * into igenerado from spioperacionlavado(4,1,imovicajaid,0,0,tdatosretrelevante);
				raise notice 'Operacion Retiro RELEVANTE generada: %',igenerado;
			end if;
			
			if trim(coalesce(tdatosrelevantecred,''))<>'' then    
				select * into igenerado from spioperacionlavado(5,1,imovicajaid,0,0,tdatosrelevantecred);
				raise notice 'Operacion Pago Cred RELEVANTE generada: %',igenerado;
			end if;
