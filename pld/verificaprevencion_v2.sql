CREATE or replace FUNCTION verificarelevantes(integer, character,integer) RETURNS text
    AS $_$
declare
	preferenciacaja alias for $1;
	pseriecaja alias for $2;
	imovicajaid alias for $3;
	filtro text;
	...
	begin
		...
		select coalesce(monedavalor,0)*10000, coalesce(monedavalor,0)*5000 into pmontorelevante,pmontoinusual from monedas where monedaid ='DL';
		select socioid,tipomovimientoid,date(fechahora) into isocioid,stipomovimientoid,dfecha from movicaja where movicajaid=imovicajaid;
		...
		if stipomovimientoid not in ('RE') then
			...
			select coalesce(sum(debe)-sum(haber),0) into pdeposito  from movicaja mc, movipolizas mp  where mc.movipolizaid=mp.movipolizaid and referenciacaja = preferenciacaja and seriecaja= pseriecaja  and efectivo in (1,4) and movicajaid=imovicajaid;
			...
			if (pdeposito >= pmontoinusual)  then 
			...
			if pdeposito > fingresos and (fingresos - (fingresos*2))>10000 then 
				if isocioid in (select socioid from socio where clavesocioint in (select clavesocioint from parametros where (trim(clavesocioint)<>'') and clavesocioint is not null)) then  
					filtro := 'PREOCUPANTES';
