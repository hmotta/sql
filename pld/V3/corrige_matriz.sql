CREATE OR REPLACE FUNCTION "sucursal1"."corrige_matriz_riesgo"()
...
BEGIN
	FOR r IN 
		SELECT	socioid,preguntaid,respuesta FROM	matrizriesgo WHERE valor = 0 	AND preguntaid = 23
	loop
	...
	FOR r IN 
		SELECT	socioid,preguntaid,respuesta FROM	matrizriesgo WHERE valor = 0 	AND preguntaid = 28
	loop
		ndias=0;
		--AÑOS
