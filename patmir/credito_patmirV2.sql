...
begin
  ...
  for r in
	...
	--tipo_garantia_adicional character varying(16),
		--'Aval',
		(case when p.clavegarantia='02' then 'GARANTIA' else (case when p.clavegarantia='02' then 'AHORRO' else (case when p.clavegarantia='03' then 'AVALES' else (case when clavegarantia='04' then 'HIPOTECA' else (case when p.clavegarantia='05' then 'DEPOSITO' else (case when clavegarantia='06' then 'AVAL' else (case when p.dias_de_cobro='07' then 'INVERSION' else 'RECIPROCIDAD' end) end) end) end) end) end) end),
	...
	where pr.fechacierre = pfecha 
		and s.tiposocioid in ('01','02','05') --modificado el 07 de septiembre del 2012, se agrego el tipo 05
		...
		and so.fechaingreso>=pfechaingreso
...
ALTER FUNCTION public.spscreditopatmir(date, date) OWNER TO sistema;

--
-- Name: spscreditopatmirc(date, date); Type: FUNCTION; Schema: public; Owner: sistema
--
